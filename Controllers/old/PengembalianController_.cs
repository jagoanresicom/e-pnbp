...
namespace Pnbp.Controllers
{
    ...
    public class PengembalianController : Controller
    {
        ...
        public ActionResult DaftarPengajuanPengembalianTrain(int? start, int? length, Entities.FindPengembalianPnbp f)
        {
            ...
            List<Entities.PengembalianPnbpTrain> result = pengembalianmodel.GetPengembalianPnbpTrain(kantoriduser, judul, namakantor, nomorberkas, kodebilling, ntpn, namapemohon, nikpemohon, alamatpemohon, teleponpemohon, bankpersepsi, status, namasatker, kodesatker, from, to);

            if (result.Count > 0)
            {
                total = result[0].Total;
            }
            //return Json(@OtorisasiUser.GetJenisKantorUser(), JsonRequestBehavior.AllowGet);
            return Json(new { data = result, recordsTotal = result.Count, recordsFiltered = total }, JsonRequestBehavior.AllowGet);
        }
        ...
        public ActionResult GetBerkasDataByID(FormCollection form)
        {
            ...
            Entities.GetDataBerkasForm result = pengembalianmodel.GetDataBerkasByNo(noberkas);
            return Json(result, JsonRequestBehavior.AllowGet);
        }
        ...
        {
            //return Json(form["pengembalianpnbpid"], JsonRequestBehavior.AllowGet);
            var ctx = new PnbpContext();
            PnbpContext db = new PnbpContext();
            var pengembalianpnbpid = ((form.AllKeys.Contains("pengembalianpnbpid")) ? form["pengembalianpnbpid"] : "");
            ...
            //        string update_lampiran = "UPDATE PNBPTRAIN.LAMPIRANKEMBALIAN SET STATUSLAMPIRAN='" + qcpusat[i] + "' WHERE LAMPIRANKEMBALIANID='" + fileid[i] + "'";
