    ...
    public class AdministrasiController : Controller
    {
        ...
        public ActionResult UpdateDataManfaatV2(Entities.PrioritasAlokasi frm)
        {
            ...
            try
            {
                ...
                var getTotalAlokasiSatker = alm.GetAlokasiSatker(frm.Kodesatker, DateTime.Now.Year);
                var getTotalTerAlokasi = alm.GetAlokasiSatkerTerAlokasi(frm.Kodesatker, DateTime.Now.Year);

                if (getTotalAlokasiSatker == 0)
                {
                    throw new Exception("Tidak ada Anggaran yg diberikan.");
                }

                var sisa = getTotalAlokasiSatker - getTotalTerAlokasi;
                //decimal totalAnggaran = Convert.ToDecimal(frm.Anggaransatker.Replace(".", ""));
                decimal totalAnggaran = frm.Nilaianggaran;
                ...
                if (frm.JUMLAHALOKASI > sisa)
                {
                    throw new Exception("Jumlah Anggaran yang diinput tidak valid, yaitu: " + string.Format("{0:#,##0}", totalAnggaran) + " <> " + string.Format("{0:#,##0}", sisa));
                }
        ...
        public ActionResult KunciDataManfaatV2(string kantorid)
        {
